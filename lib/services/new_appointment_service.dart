...
class NewAppointmentService extends ChangeNotifier {
  ...
  Future<Map<DateTime, List<NewAppointmentModel>>> getAppointmentsByDateRange({
    ...
    try {
      ...
      final appointments = querySnapshot.docs
          .map((doc) => NewAppointmentModel.fromFirestore(doc))
          .where((appointment) => appointment.isActive) // Filter active in memory
          .toList();
  ...
  Future<List<NewAppointmentModel>> getUpcomingAppointments({
    ...
    try {
      ...
      final appointments = querySnapshot.docs
          .map((doc) => NewAppointmentModel.fromFirestore(doc))
          .where((appointment) => appointment.isUpcoming && appointment.isActive)
          .toList();
  ...
  Future<List<NewAppointmentModel>> getTodaysAppointments(String veterinarianId) async {
    try {
  ...
  Future<bool> updateAppointmentStatus(String appointmentId, AppointmentStatus newStatus) async {
    try {
      await _firestore.collection('appointments').doc(appointmentId).update({
        'status': NewAppointmentModel._appointmentStatusToString(newStatus),
  ...
  Future<Map<String, dynamic>> getBookingStatistics({
    ...
    try {
      ...
      // Calculate rates
      final completionRate = totalAppointments > 0 ? (completedAppointments / totalAppointments * 100).round() : 0;
      final cancellationRate = totalAppointments > 0 ? (cancelledAppointments / totalAppointments * 100).round() : 0;
      final noShowRate = totalAppointments > 0 ? (noShowAppointments / totalAppointments * 100).round() : 0;
  ...
  Future<void> _scheduleAppointmentNotifications(String appointmentId, NewAppointmentModel appointment) async {
    try {
      ...
      if (dayBefore.isAfter(DateTime.now())) {
        await _notificationService.scheduleNotification(
          ...
          body: 'Your ${appointment.typeDisplayName.toLowerCase()} appointment is tomorrow at ${appointment.timeSlot}',
      ...
      if (hourBefore.isAfter(DateTime.now())) {
        await _notificationService.scheduleNotification(
          ...
          body: 'Your ${appointment.typeDisplayName.toLowerCase()} appointment starts in 1 hour',
  ...
  Future<void> _cancelAppointmentNotifications(String appointmentId) async {
    try {
      await _notificationService.cancelNotification(_generateNotificationId(appointmentId, '24h'));
      await _notificationService.cancelNotification(_generateNotificationId(appointmentId, '1h'));
  ...
  String? _validateAppointment(NewAppointmentModel appointment) {
    ...
    if (appointment.veterinarianId.isEmpty) return 'Veterinarian ID is required';
    if (appointment.timeSlot.isEmpty) return 'Time slot is required';
    if (appointment.reason.isEmpty) return 'Reason is required';
    if (appointment.appointmentDate.isBefore(DateTime.now())) return 'Appointment date cannot be in the past';
  ...
  Future<List<NewAppointmentModel>> searchAppointments({
    ...
    try {
      ...
      // Filter by status
      if (status != null) {
        baseQuery = baseQuery.where('status', isEqualTo: NewAppointmentModel._appointmentStatusToString(status));
      }

      // Filter by type
      if (type != null) {
        baseQuery = baseQuery.where('type', isEqualTo: NewAppointmentModel._appointmentTypeToString(type));
      }
      ...
      if (query != null && query.isNotEmpty) {
        ...
        appointments = appointments.where((appointment) {
          return appointment.reason.toLowerCase().contains(searchText) ||
                 ...
                 (appointment.medicalConcerns?.toLowerCase().contains(searchText) ?? false);
        }).toList();
      }
